...
fish \
  --init-command 'trap \'echo " '(set_color -ir bryellow)'WARN'(set_color normal)' $TMPHOME will be nuked in 10 seconds" && sh -c "sleep 10 && rm -rf \$TMPHOME" &\' EXIT' \
  ...
  --init-command '
    function fish_greeting
      ...
      echo " '(set_color -ir bryellow)'WARN'(set_color normal)' Everything in the new \$HOME will be nuked when you leave this shell!"
    end
  ' \
  --private
  # --print-rusage-self
