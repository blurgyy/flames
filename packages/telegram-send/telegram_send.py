...
async def send(*,
    ...
    if messages:
        ...
        for m in messages:
            if len(m) > MessageLimit.MAX_TEXT_LENGTH.real:
                warn(markup(
                    f"Message longer than MAX_MESSAGE_LENGTH={MessageLimit.MAX_TEXT_LENGTH.real}, splitting into smaller messages.",
                    "red"))
                ms = split_message(m, MessageLimit.MAX_TEXT_LENGTH.real)
                for m in ms:
